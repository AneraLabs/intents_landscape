...
def normalise_across_v2(original_doc, type, normalised_doc):
    if type == "tx":
        ...
        normalised_doc["destination_token_address"] = normalise_address_if_needed(
            original_doc["tx"]["destinationToken"]
        )
        ...
        normalised_doc["order_id"] = (
            ...
            + "_"
            + str(original_doc["tx"]["depositId"])
        )
        normalised_doc["protocol_fee"] = int(
            original_doc["tx"]["realizedLpFeePct"]
        ) + int(original_doc["tx"]["relayerFeePct"])
        ...
        if original_doc["scraper_event"] == "FundsDeposited":
            ...
            normalised_doc["destination_token_address"] = normalise_address_if_needed(
                original_doc["event"]["destinationToken"]
            )
            ...
            normalised_doc["protocol_fee"] = int(
                original_doc["event"]["realizedLpFeePct"]
            ) + int(original_doc["event"]["relayerFeePct"])
