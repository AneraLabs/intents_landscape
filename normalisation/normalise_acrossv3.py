...
def normalise_across_v3(original_doc, type, normalised_doc):
    if type == 'tx':
        ...
        normalised_doc['destination_address'] = original_doc['tx']['relayData']['recipient']

        normalised_doc['source_chain'] = original_doc['tx']['relayData']['originChainId']
        normalised_doc['destination_chain'] = int(normalised_doc['scraper_originChain'])

        normalised_doc['source_token_address'] = normalise_address_if_needed(original_doc['tx']['relayData']['inputToken'])
        normalised_doc['destination_token_address'] = normalise_address_if_needed(original_doc['tx']['relayData']['outputToken'])

        normalised_doc['source_token_amount'] = original_doc['tx']['relayData']['inputAmount']
        normalised_doc['destination_token_amount'] = original_doc['tx']['relayData']['outputAmount']

        normalised_doc['order_id'] = str(normalised_doc['source_chain']) + '_' + str(original_doc['tx']['relayData']['depositId'])
        normalised_doc['protocol_fee'] = int(normalised_doc['source_token_amount']) - int(normalised_doc['destination_token_amount'])
        ...
        if original_doc['scraper_event'] == 'V3FundsDeposited':
            ...
            normalised_doc['source_token_address'] = normalise_address_if_needed(original_doc['event']['inputToken'])
            normalised_doc['destination_token_address'] = normalise_address_if_needed(original_doc['event']['outputToken'])

            normalised_doc['source_token_amount'] = original_doc['event']['inputAmount']
            normalised_doc['destination_token_amount'] = original_doc['event']['outputAmount']

            # TODO double check that this is safe, maybe there is a token swap happening?
            normalised_doc['protocol_fee'] = int(original_doc['event']['inputAmount']) - int(original_doc['event']['outputAmount'])
    ...
    if 'destination_token_address' in normalised_doc and normalised_doc['destination_token_address'] == "0x0000000000000000000000000000000000000000":
